class Product:
    ...
    @property
    def profit(self):
        prof = self.retail_price - self.purchase_price

        return prof

    # Опишите статический метод average_price()
    @staticmethod
    def average_price(list_of_retail_prices=0):
        summ_of_prices = sum(list_of_retail_prices)
        number_of_stuff = len(list_of_retail_prices)
        average = summ_of_prices / number_of_stuff
